...
#define UA_CHECK_LOG_INTERNAL(STATUS_CODE, EVAL, LOG, LOGGER, CAT, MSG, ...) \
    UA_CHECK((STATUS_CODE), LOG((LOGGER), (CAT),                             \
                    "%s (%s)", ##__VA_ARGS__, UA_StatusCode_name((STATUS_CODE))); EVAL)

#define UA_CHECK_LOG(STATUS_CODE, EVAL, LOG, LOGGER, CAT, ...) \
    UA_CHECK_LOG_INTERNAL((STATUS_CODE), (EVAL), (LOG), (LOGGER), (CAT), ##__VA_ARGS__)
#define UA_CHECK_ERROR(STATUS_CODE, EVAL, LOGGER, CAT, ...) UA_CHECK_LOG( \
    (STATUS_CODE), (EVAL), UA_LOG_ERROR, (LOGGER), (CAT), ##__VA_ARGS__)
#define UA_CHECK_WARN(STATUS_CODE, EVAL, LOGGER, CAT, ...) UA_CHECK_LOG( \
    (STATUS_CODE), (EVAL), UA_LOG_WARN, (LOGGER), (CAT), ##__VA_ARGS__)
#define UA_CHECK_INFO(STATUS_CODE, EVAL, LOGGER, CAT, ...) UA_CHECK_LOG( \
    (STATUS_CODE), (EVAL), UA_LOG_INFO, (LOGGER), (CAT), ##__VA_ARGS__)
